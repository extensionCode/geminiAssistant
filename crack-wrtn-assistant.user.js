...
// @downloadURL  https://raw.githubusercontent.com/extensionCode/geminiAssistant/main/crack-wrtn-assistant.user.js
// @updateURL    https://raw.githubusercontent.com/extensionCode/geminiAssistant/main/crack-wrtn-assistant.user.js
// @noframes
// ==/UserScript==
...
(function () {
  ...
  const DEFAULTS = {
    ...
    msgCount: 20,
    memoryCount: 40,             // 요약메모리 최근 N개만 전송
    temperature: 0.9,
    // 첨부 토글
    sendPersona: true,
    sendUserNote: true,
  ...
  // 모델별 100만 토큰당 단가(USD, 추정, 2026-05 기준) — 정확 매칭 우선, 미등록은 키워드 추정
  const MODEL_PRICES = {
  ...
  function modelPricing(model) {
    ...
    for (const k in MODEL_PRICES) { if (m.indexOf(k) === 0) return MODEL_PRICES[k]; }
    // 미등록(커스텀) 모델 — 키워드로 대략 추정
    ...
    return g3 ? { in: 1.50, out: 9.00 } : { in: 0.30, out: 2.50 };   // flash
  }
  ...
  function buildUI() {
    ...
    root.innerHTML = [
      ...
      '.cwa-icon{position:fixed;right:16px;bottom:96px;width:48px;height:48px;border-radius:50%;',
      '  background:#5b9bd5;color:#fff;font-size:19px;line-height:48px;text-align:center;cursor:pointer;',
      '  box-shadow:0 4px 14px rgba(0,0,0,.28);opacity:.6;transition:opacity .15s,transform .1s;',
      '  z-index:2147483600;user-select:none;touch-action:none;}',
      '.cwa-icon:hover{opacity:1;}',
      '.cwa-icon:active{transform:scale(.92);}',
      '.cwa-panel{position:fixed;width:min(390px,93vw);max-height:min(620px,84vh);background:#fff;',
      '  border-radius:16px;box-shadow:0 10px 40px rgba(0,0,0,.35);z-index:2147483601;display:none;',
      '  flex-direction:column;overflow:hidden;color:#1f2330;}',
      '.cwa-panel.open{display:flex;}',
      '.cwa-head{display:flex;align-items:center;gap:8px;padding:12px 14px;background:#5b9bd5;color:#fff;}',
      ...
      '.cwa-body{padding:12px 14px;overflow-y:auto;display:flex;flex-direction:column;gap:10px;}',
      '.cwa-row{display:flex;align-items:center;gap:8px;font-size:12px;color:#5b6072;flex-wrap:wrap;}',
      ...
      '.cwa-btn{cursor:pointer;border:0;border-radius:10px;padding:9px 12px;font-size:13px;font-weight:700;',
      '  background:#5b9bd5;color:#fff;}',
      '.cwa-btn:hover{background:#4a89c4;}',
      ...
      '.cwa-thread{border:1px solid #e6e7f0;border-radius:10px;padding:8px;background:#fafafe;',
      '  display:flex;flex-direction:column;gap:10px;max-height:300px;overflow-y:auto;min-height:60px;}',
      ...
      '.cwa-abub{align-self:flex-start;max-width:96%;background:#fff;border:1px solid #e6e7f0;color:#1f2330;',
      ...
      '        <input type="number" id="cwa-memcount" min="1" max="999" step="1">',
      '      </div>',
    ...
    (function () {
      ...
      if (settings.iconLeft != null && settings.iconTop != null) {
        iconEl.style.left = settings.iconLeft + 'px';
        iconEl.style.top = settings.iconTop + 'px';
        iconEl.style.right = 'auto'; iconEl.style.bottom = 'auto';
      }
      iconEl.addEventListener('pointerdown', function (e) {
        dragging = true; moved = false; sx = e.clientX; sy = e.clientY;
        const r = iconEl.getBoundingClientRect(); ox = r.left; oy = r.top;
      ...
      iconEl.addEventListener('pointermove', function (e) {
        ...
        if (!moved) return;
        const nx = Math.max(0, Math.min(ox + dx, window.innerWidth - iconEl.offsetWidth));
        const ny = Math.max(0, Math.min(oy + dy, window.innerHeight - iconEl.offsetHeight));
      ...
      iconEl.addEventListener('pointerup', function (e) {
        ...
        if (moved) {
          const r = iconEl.getBoundingClientRect();
          settings.iconLeft = Math.round(r.left);
          settings.iconTop = Math.round(r.top);
    ...
    // 비용 표기 — USD + 원화 환산(실시간 환율). 너무 작으면 자릿수를 늘려 표시
    ...
    async function runTurn(turn, priorTurns) {
      ...
      turn.tokens = null; turn.cost = 0; turn.model = settings.model;
      renderThread();
